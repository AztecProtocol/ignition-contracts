...
import {Aztec, ATPFactory, IRegistry, INCATP, LockParams, Lock, LockLib, StakerVersion} from "test/token-vaults/Importer.sol";

abstract contract NCATPTestBase is Test {
    ...
    function setUp() public virtual {
        ...
        // The NCATP factory is funded with type(uint128).max tokens
    ...
    function help_upgrade(INCATP _atp, address _operator) internal {
    ...
    function help_approve(INCATP _atp, uint256 _amount) internal {
        address beneficiary = _atp.getBeneficiary();
        vm.prank(beneficiary);
        _atp.approveStaker(_amount);
    }

    function help_stake(INCATP _atp, uint256 _amount) internal {
    ...
    function help_unstake(INCATP _atp, uint256 _amount) internal {
        FakeStaker fakeStaker = FakeStaker(address(_atp.getStaker()));
        address operator = fakeStaker.getOperator();
    ...
    function help_slash(INCATP _atp, uint256 _amount) internal {
        address staker = address(_atp.getStaker());
        staking.slash(staker, _amount);
    }

    function help_getStaked(INCATP _atp) internal view returns (uint256) {
        address staker = address(_atp.getStaker());
        return staking.staked(staker);
    }

    function help_getRewards(INCATP _atp) internal view returns (uint256) {
        address staker = address(_atp.getStaker());
        return staking.rewards(staker);
    }

    function help_mintReward(INCATP _atp, uint256 _amount) internal {
        address staker = address(_atp.getStaker());
        aztec.mint(address(staking), _amount);
        staking.reward(staker, _amount);
    }

    function help_claimRewards(INCATP _atp) internal {
        FakeStaker fakeStaker = FakeStaker(address(_atp.getStaker()));
        address operator = fakeStaker.getOperator();
    ...
    function help_computeUnlocked(INCATP _atp, uint256 _time) internal view returns (uint256) {
        return _atp.getGlobalLock().unlockedAt(_time);
    }

    function help_computeAccumulated(INCATP _atp, uint256 _time) internal view returns (uint256) {
        return _atp.getAccumulationLock().unlockedAt(_time);
    }

    function help_computeUsed(INCATP _atp) internal view returns (uint256) {
    ...
    function help_log(INCATP _atp) internal {
        uint256 balance = token.balanceOf(address(_atp));

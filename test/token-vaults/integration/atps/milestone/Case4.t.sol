...
contract Case4Test is MATPTestBase {
    // In this case, we have a pending milestone for long time, and then finally the atp is revoked.
    // With the caveat that the full allocation is staked
    ...
    function test_Case4() public {
        ...
        assertEq(token.balanceOf(revoker), 0, "revoker");

        address revokerOperator = registry.getRevokerOperator();
        vm.prank(revokerOperator);
        ...
        assertEq(token.balanceOf(address(this)), 0, "this");
        assertEq(token.balanceOf(revoker), allocation, "revoker");

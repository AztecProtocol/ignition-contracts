...
contract Case2Test is MATPTestBase {
    // In this case, we have a pending milestone for long, and then finally it is marked as failed and
    // the revoker will claim the assets.
    ...
    function test_Case2() public {
        ...
        while (help_computeUnlocked(atp, block.timestamp) < token.balanceOf(address(atp))) {
            assertGt(help_computeUnlocked(atp, block.timestamp), 0, "unlocked");
        ...
        emit log_named_decimal_uint("balance(revoker) ", token.balanceOf(revoker), 18);

        emit log("Milestone failed");
        registry.setMilestoneStatus(milestoneId, MilestoneStatus.Failed);

        assertEq(atp.getClaimable(), allocation);
        assertEq(token.balanceOf(revoker), 0);
        ...
        assertEq(token.balanceOf(address(this)), 0);
        assertEq(token.balanceOf(revoker), allocation);

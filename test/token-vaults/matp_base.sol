...
import {Aztec, ATPFactory, IRegistry, IMATP, Lock, LockLib, StakerVersion} from "test/token-vaults/Importer.sol";

abstract contract MATPTestBase is Test {
    ...
    function help_upgrade(IMATP _atp) internal {
        address beneficiary = _atp.getBeneficiary();
        vm.prank(beneficiary);
        _atp.upgradeStaker(fakeStakerVersion);
    }

    function help_approve(IMATP _atp, uint256 _amount) internal {
        address beneficiary = _atp.getBeneficiary();
        vm.prank(beneficiary);
        _atp.approveStaker(_amount);
    }

    function help_stake(IMATP _atp, uint256 _amount) internal {
    ...
    function help_unstake(IMATP _atp, uint256 _amount) internal {
        FakeStaker fakeStaker = FakeStaker(address(_atp.getStaker()));
        address operator = fakeStaker.getOperator();
    ...
    function help_slash(IMATP _atp, uint256 _amount) internal {
        address staker = address(_atp.getStaker());
        staking.slash(staker, _amount);
    }

    function help_getStaked(IMATP _atp) internal view returns (uint256) {
        address staker = address(_atp.getStaker());
        return staking.staked(staker);
    }

    function help_getRewards(IMATP _atp) internal view returns (uint256) {
        address staker = address(_atp.getStaker());
        return staking.rewards(staker);
    }

    function help_mintReward(IMATP _atp, uint256 _amount) internal {
        address staker = address(_atp.getStaker());
        aztec.mint(address(staking), _amount);
        staking.reward(staker, _amount);
    }

    function help_claimRewards(IMATP _atp) internal {
        FakeStaker fakeStaker = FakeStaker(address(_atp.getStaker()));
        address operator = fakeStaker.getOperator();
    ...
    function help_computeUnlocked(IMATP _atp, uint256 _time) internal view returns (uint256) {
        return _atp.getGlobalLock().unlockedAt(_time);
    }

    function help_computeUsed(IMATP _atp) internal view returns (uint256) {
    ...
    function help_log(IMATP _atp) internal {
        uint256 balance = token.balanceOf(address(_atp));

...
Genesis sequencer refers to whitelisted addresses that will take place in the initial fixed price sale.
Contributor refers to whitelisted addresses that will be able to take part in the earliest auction stages.
General participants are not part of any whitelist group, but will be able to take part in the public sale - happening after the exclusive contributor period ends.

Participation in the initial fixed-price sale and the auction is restricted to users who possess an IgnitionParticipant Soulbound NFT.
...
ZKPassport allows the user to create a local proof that they hold a valid passport, and that the name, passport number and birthdate combination do not appear on a sanctions list (OFAC + Swiss + EU + UK for now). It will also invoke liveness checks using the mobile's secure element to attest that the passport holder's image is close to a photo taken at the time by the user.
...
_Predicate_  
The second provider will be using https://predicate.io/ (Identity product). As zk passport does not support all jurisdictions, we offer a second provider that allows them to perform a traditional KYC check, using a software vendor called https://sumsub.com/.
...
Identity Providers are whitelisted by the contract owner. The user supplies the address of the identity provider they would wish to use, but will only be able to use those which the contract owner has prescribed (zkpassport + predicate).
...
However; these tokens are not directly transferred to the user, instead they mint an Aztec Token Position (ATP) also referred to as an Aztec Token Vault.

- It has a beneficiary who will receive the tokens contained in the ATP.
- Tokens are freely transferrable upon TGE.
- The tokens can be used in either staking or governance even before TGE.
...
Operators who take part in the Sale are held to specific terms, and in particular MUST stake the purchased tokens. If they do not perform the stake operation, they cannot withdraw. Linear ATPs do not have this restriction.

Although tokens remain non-transferrable until TGE, they can be used for staking and governance via the ATP Staking Extension, but for nothing else. All genesis sale and auction participants who purchased more than 200k tokens will receive an NCATP.

The variants have their intentions in common, they exist to:

1. Distribute tokens to beneficiaries (individuals, companies, etc.)
2. Ensure that tokens are non-transferrable until TGE
3. Provide a mechanism for the beneficiary (the recipient of the tokens) to use at least some portion of their tokens in staking or governance **before** the tokens are transferrable but **after** some specified time.
4. Provide a mechanism for the ATP Registry owner to revoke the ATP in case of misconduct, and claw back the tokens that have not yet been `accumulated`.

_Note in the case of the Aztec Sale, only Aztec insiders have ATPs that are revokable._

With the most important being 1 and 2.

## Global Unlock Schedule

Common for MATP and LATP variants is that point 2 is enforced by the same global unlock schedule.
...
Since the network will be in its infancy it is decided that it will be possible for ATPs to use not-yet transferrable assets in certain approved operations under certain conditions - such as staking.
...
To support this, each ATP, be it Milestone, Linear or Non Claimable is paired with its own Staker contract.
...
This separation ensures the beneficiary deals with token ownership, while a separate operator wallet handles staking details such as attestation keys. Note the operator can be the same address as the beneficiary.

The owner of the Registry contract (whitelist) maintains a set of staker implementations, that can be used by the ATPs.
...
The implementation address is required to be previously whitelisted by the Owner of the Registry, to guard against malicious Staker contracts.

![Upgrade](figures/operator-upgrade.png)

Unstaking is triggered by the operator via a call to the Staker.
The Staker implementation, having been vetted by the beneficiary, immediately sends the unstaked funds back to the ATP contract.
...
_Note only Aztec insiders receive MATPs, all sale participants receive either NCATPs or non-revokable LATPs_
...
Failed could for example happen if there is a deadline that was not met, or an employee leaves employment.
...
If the milestone is marked as failed the effect is as if the MATP was revoked.
...
Linear Aztec Token Positions exist in both revokable and non-revokable variants.
...
In the case where Alice does something that is against the rules (tbd), the allocation that she has not yet accumulated is revoked.
...
As the genesis sale participants will receive ATPs, and as they are expected to stake, we need to make it possible for the ATP holders to actually stake the tokens which they hold.

Each ATP is paired with its own Staker contract whose whitelisted implementations are managed by the Registry. The ATP is managed by the beneficiary, while it can set its Staker to be managed by an operator address.
The implementations of approved Staker contracts are managed by a Registry of staker implementations. The owner of the Registry contract is the only entity capable of approving new Staker implementations.

_For auction participants, the owner of the Registry is the Aztec Governance which means a governance proposal is required to approve new Staker implementations_

![Stake](figures/operator-stake.png)

It is expected that participants who take part in the Genesis Sequencer Sale are agreeing to run a sequencer on the network when it launches. This may be before there is a liquid market for the Aztec Token, and so it is important that the Staker implementation controls the flow of funds.
...
This will allow the ATP beneficiary to set their staker version.
...
**Staking Registry**
Users who run their own aztec nodes will be able to stake to them directly. However, we do not expect all people who take part in the genesis sale or in the later auction to run their own nodes.

The Aztec system does NOT support delegated proof of stake as a first class primitive. However we do want to offer a place where token holders can delegate sequencer operations to a third party.
...
When users initiate a deposit with a provider, a [0xSplits](https://docs.splits.org/core/split-v2) Pull Splitter contract will be deployed managing the user's fee recipient with the provider's fee recipient.
...
40% of the sale supply will be sold in the last block to prevent final clearing price manipulation. ( People bid over the remaining supply, so more supply at the end would have a much larger impact on clearing price - you do not want this to be extreme as it will mean more raised currency must be used for the pool ).
...
This pool has a beforeTrading hook that will not be active until Aztec Governance votes to enable it. Once enabled - Governance cannot disable trading.

Ownership of the pool is held by a `GovernanceAcceleratedLock` (which is in turn owned by Governance). After 90 days from the beginning of the sale, Governance can activate trading on the pool by triggering the boolean.
...
- Soulbound minters should take care to validate that they are minting for their highest privilege. As once they have minted they will not be able to upgrade their token.
...
- Since the `Staker` is a proxy there was a risk of self-destructing the implementation or proxy. However, since the cancun fork [EIP-6780](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-6780.md) made self-destruct only possible to use in the same transaction as in which the contract was created, this is not a concern.
- We add a `rescueFunds` function to the `ATP` contract to allow the `beneficiary` to rescue funds that are not Aztec tokens from the contract.
- The `Aztec` token contract is a simple `Ownable` `ERC20` contract from OpenZeppelin that allows the `owner` to mint tokens.
- The `Registry` is essentially the "source of truth" for values shared across the ATP's, it is ownable.
  ...
  - Certain time-base values (i.e. `unlockStartTime`) can only be decreased from their initial values. This is to ensure that tokens that could have been claimed are not suddenly locked.
...
- If the `RegistryOwner` is malicious, they might add a bad staker implementation, to steal staked funds, but the `beneficiary` will need to upgrade to it for that to work. Alternatively, the `RegistryOwner` might simply never add a staker implementation, and then it is just good ol' unlocking.
...
- Spreading a beneficiary's allocation across multiple `MATP`s could mean each individual MATP allocation ends up below the minimum staking threshold, even if the total beneficiary allocation combined across their `MATP`s would be enough to participate in staking. This can be worked around by introducing an intermediate LST-like staking contract that aggregates funds across the beneficiary's `MATP`s.
